...
from mlb_pitchers import get_probable_pitcher
from injuries_client import get_injury_notes
from ai_analyzer import analyze_game

app = FastAPI(title="Today's ATS/ML/Totals Picks")
...
async def get_picks(
    ...
    snapshot_only: bool = Query(False, description="if true, only fetch odds and record line-movement snapshots - skips the Claude API entirely. Use this for scheduled/cron pings so they don't burn Anthropic credits."),
):
    """
    Fetch upcoming games for the requested sport(s), snapshot the lines,
    ...
    for sport_key in sport_keys:
        ...
        for event in events:
            ...
            if sport_key == "baseball_mlb":
                ...
                summary["pitchers"] = await get_probable_pitcher(
                    summary["home_team"], summary["away_team"], summary["commence_time"]
                )
            else:
                summary["weather"] = await get_game_weather(summary["home_team"])
                summary["pitchers"] = None
