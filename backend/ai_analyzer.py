...
evidence-grounded ATS/ML/Totals pick with a confidence score.

HONESTY NOTE: "confidence score" means "how strongly the model's stated
reasoning supports this given the data it was given" - NOT a calibrated
real-world win probability. Treat every output as one analytical opinion
among many inputs, not a lock.
"""
import json
import re
...
odds data, line movement, probable starting pitchers (MLB only, when available), \
and any available injury/weather context for one game. Every game has a side the \
market favors (even if only slightly), and a total that's either more likely to go \
over or under - your job is to pick whichever single angle (ATS, ML, or Totals) you \
find most compelling for this specific game, name it, and give an honest confidence \
score for how strong that edge actually is. You always name a pick - the confidence \
score is what carries all the honesty about how strong or weak that edge is.
...
- Choose whichever of ATS, ML, or Totals has the most compelling case for THIS game. \
For MLB specifically, Totals (Over/Under) is often the better angle when wind is \
classified as blowing out/in, or when both probable pitchers' ERAs strongly suggest \
a high or low scoring game - don't default to ATS/ML out of habit when Totals data \
is stronger for a given matchup. For "team" on a Totals pick, use "Over" or "Under".
- Be conservative on the confidence NUMBER, not on whether to name a pick. Most games \
...
records, stats, or trends you were not given. If you don't have enough information to \
judge something, say so explicitly rather than guessing, and let that uncertainty \
pull your confidence down rather than making you omit a pick.
...
- When probable pitcher data is given, weigh it heavily - starting pitcher quality \
is one of the biggest drivers of both moneyline value and run totals in baseball. \
A significant ERA gap between the two starters is a real, legitimate signal.
...
JSON schema:
{
  "pick_type": "ATS" | "ML" | "Totals",   // always populated
  "team": string,                          // team name, or "Over"/"Under" for Totals
  "line": string,                          // e.g. "-3.5", "+150", or "8.5"
...
def _format_pitcher(label: str, pitcher: dict | None) -> str:
    if not pitcher:
        return f"{label} probable pitcher: not yet announced or unavailable."
    stats = pitcher.get("stats")
    if not stats:
        return f"{label} probable pitcher: {pitcher.get('name')} (season stats unavailable)."
    return (
        f"{label} probable pitcher: {pitcher.get('name')} - "
        f"ERA {stats.get('era')}, WHIP {stats.get('whip')}, "
        f"{stats.get('wins')}-{stats.get('losses')}, "
        f"{stats.get('strikeouts')} K in {stats.get('innings_pitched')} IP."
    )
...
def _build_user_prompt(game: dict) -> str:
    ...
    if pitchers:
        parts.append(_format_pitcher("Home", pitchers.get("home")))
        parts.append(_format_pitcher("Away", pitchers.get("away")))
    ...
    if injuries:
        parts.append(f"Injury notes: {injuries}")
    else:
...
def _extract_json(text: str) -> dict:
    ...
    try:
        ...
        # Fall back to grabbing the outermost {...} block, in case there's
...
async def _call_claude_once(game: dict, retry_note: str = "") -> str:
    prompt = _build_user_prompt(game)
    if retry_note:
        prompt += f"\n\n{retry_note}"
...
async def analyze_game(game: dict) -> dict:
    """Call Claude for a single game and return the parsed pick dict. Retries
    once on a parse failure before giving up, and logs the actual raw
    response text (not just the exception) so failures are debuggable."""
    ...
    for attempt in range(2):
        try:
            retry_note = (
                ...
                "Output ONLY the JSON object, nothing else - no markdown, no commentary."
                if attempt == 1 else ""
            )
            raw_text = await _call_claude_once(game, retry_note)
            return _extract_json(raw_text)
        except (json.JSONDecodeError, Exception) as e:  # noqa: BLE001
            last_error = e
            preview = raw_text[:300] if raw_text else "(empty response)"
            print(f"[ai_analyzer] attempt {attempt + 1} FAILED for {matchup}: {repr(e)} | raw preview: {preview!r}")
    ...
    return {
        ...
        "confidence": 0,
        "key_factors": [],
        "reasoning": f"Analysis failed after retry: {last_error}",
        "_error": True,
    }
